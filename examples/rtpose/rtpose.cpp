...
DEFINE_string(write_frames, "D:/xqg/data/", "Write frames with format prefix%06d.jpg");
DEFINE_bool(no_frame_drops, true, "Dont drop frames.");
DEFINE_string(write_json, "D:/xqg/data/", "Write joint data with json format as prefix%06d.json");
DEFINE_int32(camera, 0, "The camera index for VideoCapture.");
DEFINE_string(video, "", "Use a video file instead of the camera.");
DEFINE_string(image_dir, "D:/xqg/data/", "Process a directory of images.");
...
std::vector<vector<Point2f> > all_joints;
std::vector<vector<float>> all_joints_confidence;
int frame_counter_write = 0;
int frame_counter_reaad = 0;

// Global parameters
int DISPLAY_RESOLUTION_WIDTH =480;// 1920;
int DISPLAY_RESOLUTION_HEIGHT = 270;// 1080;
...
int joints_link_back[14][2] = { { 1, 16 }, { 1, 17 }, { 1, 2 }, { 2, 3 }, { 3, 4 }, { 1, 5 }, { 5, 6 }, { 6, 7 }, { 1, 8 }, { 1, 11 }, { 8, 9 }, { 9, 10 }, { 11, 12 }, { 12, 13 } };

vector<int> front_back_flag;
int use_texture = 1;
...
static int load_colorImage_frame_counter = 0;
template <class TYPE>
void Update(TYPE t)
{
...
void Handle_Display2()
{
	...
	//if (frame_id < frame_counter_write)
	if (frame_id < kinect_read_frame_counter && frame_id < frame_counter_write - 1 && frame_id < load_colorImage_frame_counter - 1){
		//clock_t begin = clock();
		Sleep( 50);
		Mat depth = depthImagesQueue[frame_id];
		Mat mask_origin = maskImagesQueue[frame_id];
		Mat texture_origin = ColorImagesQueue[frame_id];
		cv::flip(texture_origin, texture_origin, 0);
		cv::cvtColor(texture_origin, texture_origin, CV_BGR2RGB);
		vector<Point2f> joints = all_joints[frame_id];
		vector<float>   joints_confidence = all_joints_confidence[frame_id];
		...
		if (use_texture != 0)
		{
			...
			glPixelStorei(GL_PACK_ALIGNMENT, (texture_origin.step & 3) ? 1 : 4);
			glPixelStorei(GL_PACK_ROW_LENGTH, texture_origin.step / texture_origin.elemSize());
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
				GL_NEAREST);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture_origin.cols,
				texture_origin.rows, 0, GL_RGB, GL_UNSIGNED_BYTE,
				texture_origin.data);
		...
		MESH<FLOAT_TYPE> mesh_clean;
		int fast_skeleton_only = 0;

		if (fast_skeleton_only == 1){
			...
			//////////////////////////////////////RemoveLong Face
			RemoveLongFace(mesh_new, 30, mesh_clean);
			...
			// unproject2DPoints
			int use_intersect_flag = 0;


			if (use_intersect_flag == 1){
				Mat proj = KK*Rt;
				...
				for (int i = 0; i < dir.rows; i++)
				{
					...
					mesh_clean.Select(p, q, temp_select_v);
					//	cout << temp_select_v << endl;
					intersecetion_ind[i] = temp_select_v;
					if (temp_select_v != -1)
					{
						joints_dst2[i].x = mesh_clean.X[temp_select_v * 3 + 0];
						joints_dst2[i].y = mesh_clean.X[temp_select_v * 3 + 1];
						joints_dst2[i].z = mesh_clean.X[temp_select_v * 3 + 2];
					}
			...
			for (int i = 0; i < joints.size(); i++)
			{
				...
				if (i != 16 && i != 17  && joints[i].y != 0 && joints_confidence[i]>0.3){
					mesh_clean.Make_Sphere(joints_dst3[i].x, joints_dst3[i].y, joints_dst3[i].z, 14, 20, 20);
				}
			}

			//	cout << mesh_new.number << endl;

			mesh_clean.Scale(0.001);
			//OPENGL_DRIVER myRender(mesh);

			FLOAT_TYPE min_x, min_y, min_z;
			FLOAT_TYPE max_x, max_y, max_z;
			mesh_clean.Range(min_x, min_y, min_z, max_x, max_y, max_z);
		...
		//	cout <<"joints_confidence :"<< joints_confidence[i] << endl;

			if (joints[i].x != 0 && joints[i].y != 0 && joints_confidence[i]>0.3){
				glPushMatrix();
				glColor3f(0, 1, 0);
		...
		if (front_back_flag[frame_id] == 0){
			...
			for (int i = 2; i < 17; i++){
				...
				if ((joints_dst3[joints_links[i][0]].x != 0) && (joints_dst3[joints_links[i][1]].x != 0) && (joints_dst3[joints_links[i][1]].y != 0) && (joints_dst3[joints_links[i][0]].y != 0)){


				
					//	glDisable(GL_DEPTH_TEST);
					if (joints_confidence[joints_links[i][0]]>0.3 && joints_confidence[joints_links[i][1]] > 0.3){
						//	cout << distance << endl;
						glColor3f(1, 0, 1);
						glBegin(GL_LINES);
			...
			for (int i = 2; i < 14; i++){
				...
				//	glDisable(GL_DEPTH_TEST);
				if ((joints_dst3[joints_link_back[i][0]].x != 0) && (joints_dst3[joints_link_back[i][1]].x != 0) && (joints_dst3[joints_link_back[i][1]].y != 0) && (joints_dst3[joints_link_back[i][0]].y != 0)){
					if (joints_confidence[joints_link_back[i][0]]>0.3 && joints_confidence[joints_link_back[i][1]] > 0.3){
						
						glColor3f(1, 0, 1);
						glBegin(GL_LINES);

						glLineWidth(200.0);
						//	cout << joints_dst3[joints_link_back[i][0]].x << endl;


						glVertex3f(joints_dst3[joints_link_back[i][0]].x / 1000, -joints_dst3[joints_link_back[i][0]].y / 1000, joints_dst3[joints_link_back[i][0]].z / 1000 + 0.1);
						glVertex3f(joints_dst3[joints_link_back[i][1]].x / 1000, -joints_dst3[joints_link_back[i][1]].y / 1000, joints_dst3[joints_link_back[i][1]].z / 1000 + 0.1);
						glEnd();
					}
				}
				//	glEnable(GL_DEPTH_TEST);
	...
	//	cout << "kinect read in " << kinect_read_frame_counter <<" frames." << endl;
		/*depthImagesQueue.erase(depthImagesQueue.begin() + frame_id);
		ColorImagesQueue.erase(ColorImagesQueue.begin() + frame_id);
		maskImagesQueue.erase(maskImagesQueue.begin() + frame_id);
		all_joints.erase(all_joints.begin() + frame_id);
		front_back_flag.erase(front_back_flag.begin() + frame_id);

		kinect_read_frame_counter--;
		frame_counter_write--;
		load_colorImage_frame_counter--;*/
...
void Handle_Idle()
{
	...
	//if (frame_id < frame_counter_write-1){
	if (frame_id < kinect_read_frame_counter - 1 && frame_id < frame_counter_write - 1){
		frame_id = ((frame_id + 1));
	//	Sleep(100); // xqg
	}
...
void OPENGL_DRIVER()
{
	...
	//WIN32_FIND_DATA fd;
	//LPCWSTR txtFolder = L"E:/skeletonTracking/jointsLocation/*.txt";

	//HANDLE hFind = FindFirstFile(txtFolder, &fd);
	...
	for (int i = 0; i <300; i++)
	{
		...
		sprintf(colorFullName, colorName, i);

		Mat textureImage = imread(colorFullName, CV_LOAD_IMAGE_COLOR);   // Read the file

		char aName[128] = "C:\\trackingData\\Data\\depth\\frame_%04d_depth.pgm";
		char aNameDepth[128];
		sprintf(aNameDepth, aName, i);


		char maskName[128] = "C:\\trackingData\\Data\\mask\\frame_%04d_mask.bmp";
		char aNameMask[128];
		sprintf(aNameMask, maskName, i);
		...
		Mat depth = imread(aNameDepth, CV_LOAD_IMAGE_ANYDEPTH);   // Read the file
		...
		Mat mask_origin = imread(aNameMask);   // Read the file
		...
		//		Point2f aPoint;
		////		std::vector< std::vector<float>> oneFrame = all_joints[i];
		//		for (int j = 0; j < 18; j++)
		//		{
		//			joints.clear();
		//			aPoint.x = oneFrame[j][0];
		//			aPoint.y = oneFrame[j][1];
		//		}
		//
		//		joints.push_back(aPoint);
...
void warmup(int device_id) {
	...
	LOG(ERROR) << "Warming up time: " << total_time_sec << " seconds.";
}
...
void* getFrameFromCam(void *i) {
	...
	while (load_colorImage_frame_counter < kinect_read_frame_counter - 1)
	{
		continue;
	}
	...
	//	if (load_colorImage_frame_counter < 500 && load_colorImage_frame_counter < kinect_read_frame_counter - 1){
		if (load_colorImage_frame_counter < kinect_read_frame_counter - 1){
		...
		//	image_uchar_orig = cv::imread("C:\\TrackingData2\\color\\frame_0162.bmp", CV_LOAD_IMAGE_COLOR);
		
			//cout << image_uchar_orig.type() << endl;
			image_uchar_orig = ColorImagesQueue[load_colorImage_frame_counter].clone();

		//	DISPLAY_RESOLUTION_WIDTH =  image_uchar_orig.cols;
		//	DISPLAY_RESOLUTION_HEIGHT = image_uchar_orig.rows;
				cvtColor(image_uchar_orig, image_uchar_orig, CV_BGRA2BGR);
			//	cout << image_uchar_orig.type() << endl;
			//	image_uchar_orig = ColorImagesQueue.front().clone();
		//	namedWindow("Display window3", WINDOW_AUTOSIZE);// Create a window for display.
			//imshow("Display window3", image_uchar_orig);
			//waitKey(10);
		...
		//	cv::Mat wrap_frame2(DISPLAY_RESOLUTION_HEIGHT, DISPLAY_RESOLUTION_WIDTH, CV_8UC3, frame.data_for_mat);
		//	namedWindow("Display window2", WINDOW_AUTOSIZE);// Create a window for display.
		//	imshow("Display window2", wrap_frame2);
		//	waitKey(10);

			load_colorImage_frame_counter++;
			//ColorImagesQueue.push_back(image_uchar_orig.clone());
			//depthImagesQueue.push_back(depth_image.clone());
			//maskImagesQueue.push_back(mask_image.clone());
...
void* processFrame(void *i) {
	...
	std::cout << "In the processFrame method." << std::endl;
	int tid = *((int *)i);
	warmup(tid);
	...
	while (1) {
		...
		if (kinect_read_frame_counter > 0 && load_colorImage_frame_counter > 0){
			if (global.quit_threads)
				break;
			...
			while (valid_data < 1) {
				...
				if (global.input_queue.size() > 0) {

					global.input_queue.try_pop(&frame);
			...
			vector<Point2f> joints_temp;
			vector<float> joints_temp_confidence;
			for (int j = 0; j < 18; j++)
			{

				float x = scale * joints[0 + j * 3];
				float y = scale * joints[1 + j * 3];
				float c = scale * joints[2 + j * 3];
				joints_temp.push_back(Point2f(x, y));
				joints_temp_confidence.push_back(c);
				//a_joint[0] = scale * joints[0 + j * 3];
				//a_joint[1] = scale * joints[1 + j * 3];
				//a_joint[2] = scale * joints[2 + j * 3];

				//one_frame_joints.push_back(a_joint);
			}
			all_joints.push_back(joints_temp);
			all_joints_confidence.push_back(joints_temp_confidence);
			if (joints_temp[15].x == 0 && joints_temp[15].y == 0){
				//	cout << i << " th back" << "  " << textureName << endl;
				front_back_flag.push_back(1);
			}
			else{

				front_back_flag.push_back(0);
			}
		...
		//std::vector<float> a_joint(3);
		//std::vector< std::vector<float> > one_frame_joints;
		//one_frame_joints.clear();
		//for (int j = 0; j < 18; j++)
		//{
		//	a_joint[0] = scale * joints[0+j*3];
		//	a_joint[1] = scale * joints[1+j*3];
		...
		//	one_frame_joints.push_back(a_joint);
		//}
		//all_joints.push_back(one_frame_joints);

		//std::cout << "for frame " << count << " :" << scale * joints[0] << " " << scale * joints[1] << " " << joints[2] << std::endl; // need to * scale to be the  same as output
...
void* buffer_and_order(void* threadargs) { //only one thread can execute this
	...
	std::cout << "In the buffer_and_order method." << std::endl;
	FrameCompare comp;
...
void* postProcessFrame(void *i) {
	...
	std::cout << "In the postProcessFrame method." << std::endl;
	//int tid = *((int *) i);
	Frame frame;
...
void* displayFrame(void *i) { //single thread
	std::cout << "In the displayFrame method." << std::endl;
	Frame frame;
	int counter = 1;
...
void* readData(void *){
	...
	std::cout << "In the readData method." << std::endl;
...
void* inputDatatoGlobal(void *){
	std::cout << "In the inputDatatoGlobal method." << std::endl;
	int global_counter = 1;
	int frame_counter = 0;
	cv::Mat image_uchar;
	cv::Mat image_uchar_orig;
	...
	while (1){
		...
		if (!ColorImagesQueue.empty()){
			...
			//	cv::imshow("Display window", ColorImagesQueue.front());
			//	cv::imshow("Depth image", depthImagesQueue.front());
			//	cv::waitKey(10);

			image_uchar_orig = ColorImagesQueue.front().clone();
			//	image_uchar_orig = cv::imread("D:/xqg/caffe_rtpose-master/test.PNG", 1);
			DISPLAY_RESOLUTION_WIDTH = image_uchar_orig.cols;
			DISPLAY_RESOLUTION_HEIGHT = image_uchar_orig.rows;
			...
			//cv::Mat wrap_frame(DISPLAY_RESOLUTION_HEIGHT, DISPLAY_RESOLUTION_WIDTH, CV_32FC3, frame.data_for_mat);
			//namedWindow("Display window2", WINDOW_AUTOSIZE);// Create a window for display.
			//imshow("Display window2", image_uchar);
			//waitKey(10);
			//pad and transform to float
			int offset = 3 * NET_RESOLUTION_HEIGHT * NET_RESOLUTION_WIDTH;
			frame.data = new float[BATCH_SIZE * offset];
			...
			if (cnt++ < 10)
			{
				//cout << frame.ori_width << endl;
			}
			//		cout << frame.ori_width << endl;

			//Frame frameNew;
			//global.input_queue.try_pop(&frameNew);
			//cout << frameNew.ori_width << endl;
			//cout << frame.ori_width << endl;

			//			ColorImagesQueue.pop();
			//			depthImagesQueue.pop();
		}
...
void* OPENGL_DRIVER2(void *i){
	...
	for (int pp = 0; pp < 300; pp++)
	{
		...
		while (valid_data < 1) {
			...
			if (global.input_queue.size() > 0) {
				...
				//depth_image = depthImagesQueue.front().clone();
				//mask_image = maskImagesQueue.front().clone();
				//color_image = ColorImagesQueue.front().clone();
				//	namedWindow("Display window5", WINDOW_AUTOSIZE);// Create a window for display.
				//	imshow("Display window5", depth_image);                   // Show our image inside it.
	...
	glutDisplayFunc(Handle_Display2);
	glutReshapeFunc(Handle_Reshape);
	glutKeyboardFunc(Handle_Keypress);
	//glutMouseFunc(Handle_Mouse_Click);
	//glutMotionFunc(Handle_Mouse_Move);
...
void* OPENGL_DRIVER3(void *i){
	...
	glutInitWindowPosition(50, 50);
	glutInitWindowSize(screen_width, screen_height);
	glutCreateWindow("Deep 3D Body Tracking");
	glutDisplayFunc(Handle_Display2);
	glutReshapeFunc(Handle_Reshape);
	glutKeyboardFunc(Handle_Keypress);
	//glutMouseFunc(Handle_Mouse_Click);
...
int rtcpm() {
	...
	std::vector<pthread_t> threads_pool_kinect(thread_pool_size);

	for (int i = 0; i < thread_pool_size; i++) {
		int *arg = new int[i];
		int rc = pthread_create(&threads_pool_kinect[i], NULL, readData, (void *)arg);
		if (rc) {
			LOG(ERROR) << "Error: unable to create readData thread," << rc << "\n";
			return -1;
		}
	}
	...
	/*std::vector<pthread_t> gpu_threads_pool_2(NUM_GPU);
	for (int gpu = 0; gpu < NUM_GPU; gpu++) {
		int *arg = new int[1];
		*arg = gpu + FLAGS_start_device;
		int rc = pthread_create(&gpu_threads_pool_2[gpu], NULL, processFrame2, (void *)arg);
		if (rc) {
			LOG(ERROR) << "Error:unable to create thread," << rc << "\n";
			return -1;
		}
	...
	std::vector<pthread_t> gpu_threads_pool2(NUM_GPU);
	for (int gpu = 0; gpu < NUM_GPU; gpu++) {
		int *arg = new int[1];
		*arg = gpu + FLAGS_start_device;
		int rc = pthread_create(&gpu_threads_pool2[gpu], NULL, OPENGL_DRIVER3, (void *)arg);
		if (rc) {
			LOG(ERROR) << "Error:unable to create thread," << rc << "\n";
			return -1;
		}
	...
	//int *arg_opengl = new int[1];
	//int rc = pthread_create(&openGLThread, NULL, renderOpenGL, (void*)arg_opengl);
	//if (rc) {
	//	LOG(ERROR) << "Error: unable to create openGL thread," << rc << "\n";
	//	return -1;
	//}
	...
	for (int i = 0; i < thread_pool_size; i++) {
		pthread_join(threads_pool_kinect[i], NULL);
	}
	...
	/*for (int i = 0; i < NUM_GPU; i++) {
		pthread_join(gpu_threads_pool_2[i], NULL);
	}*/

	for (int i = 0; i < NUM_GPU; i++) {
		pthread_join(gpu_threads_pool2[i], NULL);
	}
