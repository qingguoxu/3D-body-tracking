...
int rtcpm() {
   ...
   // pthread_t gpu_threads_pool[NUM_GPU];
	...
	int thread_pool_size_xqg = 1;
	//pthread_t threads_pool[thread_pool_size];
	std::vector<pthread_t> threads_pool_xqg(thread_pool_size_xqg);

	for (int i = 0; i < thread_pool_size_xqg; i++) {
		int *arg = new int[i];
		int rc = pthread_create(&threads_pool_xqg[i], NULL, renderOpenGL, (void *)arg);
		if (rc) {
			LOG(ERROR) << "Error: unable to create thread to run opengl," << rc << "\n";
			return -1;
		}
	}
	VLOG(3) << "Finish spawning " << thread_pool_size_xqg << " threads. now waiting." << "\n";
...
xqg*/
	...
	for (int i = 0; i < thread_pool_size_xqg; i++) {
		pthread_join(threads_pool_xqg[i], NULL);
	}
